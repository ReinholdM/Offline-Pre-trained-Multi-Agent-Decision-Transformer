...
class GPT(nn.Module):
    ...
    def forward(self, states, pre_actions, rtgs=None, timesteps=None):
        ...
        # todo(Muning) check here =============== begin
        state_embeddings = self.state_encoder(
            states.reshape(-1, self.state_size).type(torch.float32).contiguous())  # (batch * block_size/btach * other_number, n_embd)
        state_embeddings = state_embeddings.reshape(states.shape[0], states.shape[1],
                                                    self.config.n_embd)  # (batch, block_size, n_embd)
        # todo(Muning) check here =============== end

        if pre_actions is not None and self.model_type == 'reward_conditioned':
            # todo(Muning) check here =============== begin
            rtg_embeddings = self.ret_emb(rtgs.type(torch.float32)) # cancle the unsqueeze(-1)
            ...
            # todo(Muning) check here =============== end
        elif pre_actions is None and self.model_type == 'reward_conditioned':  # only happens at very first timestep of evaluation
        ...
        elif pre_actions is not None and self.model_type == 'naive':
            action_embeddings = self.action_embeddings(
                pre_actions.type(torch.long).squeeze(-1))  # (batch, block_size, n_embd)
        ...
        elif pre_actions is None and self.model_type == 'naive':  # only happens at very first timestep of evaluation
        ...
        position_embeddings = torch.gather(all_global_pos_emb, 1,
                                           torch.repeat_interleave(timesteps.unsqueeze(-1), self.config.n_embd,
                                                                   dim=-1)) + self.pos_emb[:,
                                                                              : token_embeddings.shape[1], :] # position embedding for the state and reward is same for the repeat operation
        # todo(Muning) check here =============== end
        ...
        elif pre_actions is None and self.model_type == 'reward_conditioned':
            logits = logits[:, 1:, :]
        elif pre_actions is not None and self.model_type == 'naive':
            logits = logits[:, ::2, :]  # only keep predictions from state_embeddings
        elif pre_actions is None and self.model_type == 'naive':
